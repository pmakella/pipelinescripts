...
#   make Tophat alignment config file
    ...
    cp /glusterfs/users/pmakella/scripts/run_Tophat_alignment.mouse.bid.ini  $dirname/run_Tophat_alignment.$BID.ini
    ...
    sed -i 's/INPUTPATH/'"$var2"'/g'  $dirname/run_Tophat_alignment.$BID.ini
#END
...
#   make Quantification config file
    cp /glusterfs/users/pmakella/scripts/run_RNAseq_expr_analysis.mouse.bid.ini  $dirname/run_RNAseq_expr_analysis.$BID.ini
    sed -i 's/BID/'"$BID"'/g' $dirname/run_RNAseq_expr_analysis.$BID.ini
    sed -i 's/RUNDATE/'"$RunDate"'/g' $dirname/run_RNAseq_expr_analysis.$BID.ini
    sed -i 's/SAMPLE/'"$Sample"'/g' $dirname/run_RNAseq_expr_analysis.$BID.ini
    outpath=$dirname/$BID/exome
    var1=$(echo $outpath | awk '{gsub(/\//,"\/")}; $1')
    set -- $var1
    sed -i 's/OUTPATH/'"$var1"'/g'  $dirname/run_RNAseq_expr_analysis.$BID.ini
    ...
    set -- $var2
    sed -i 's/INPUTPATH/'"$var2"'/g'  $dirname/run_RNAseq_expr_analysis.$BID.ini


#whole genome
    cp /glusterfs/users/pmakella/scripts/run_RNAseq_expr_analysis_wholegenome.mouse.bid.ini  $dirname/run_RNAseq_expr_analysis_wholegenome.$BID.ini

    sed -i 's/BID/'"$BID"'/g' $dirname/run_RNAseq_expr_analysis_wholegenome.$BID.ini
    sed -i 's/RUNDATE/'"$RunDate"'/g' $dirname/run_RNAseq_expr_analysis_wholegenome.$BID.ini
    sed -i 's/SAMPLE/'"$Sample"'/g' $dirname/run_RNAseq_expr_analysis_wholegenome.$BID.ini

#    `mkdir $dirname/$BID/exome`
    outpath=$dirname/$BID/wholegenome
    var1=$(echo $outpath | awk '{gsub(/\//,"\/")}; $1')
    set -- $var1
    sed -i 's/OUTPATH/'"$var1"'/g'  $dirname/run_RNAseq_expr_analysis_wholegenome.$BID.ini

    inputpath=$dirname/$BID/
    var2=$(echo $inputpath | awk '{gsub(/\//,"\/")}; $1')
    set -- $var2
    sed -i 's/INPUTPATH/'"$var2"'/g'  $dirname/run_RNAseq_expr_analysis_wholegenome.$BID.ini
