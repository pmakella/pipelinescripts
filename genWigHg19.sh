...
if [ $# -ne 2 ]
then
echo "Usage : genWigHg.sh <dirname>  <sequencedatafilename> "
exit 1
fi
...
/glusterfs/users/malijia/source/bwa-0.5.9/bwa aln -I -B 0 -t 4 /glusterfs/users/pmakella/RSQ6files/hg/refFiles/hg19.fa  $DataFile > $Dirname/$DataFile1.sai


#echo "created .sai file" 

/glusterfs/users/malijia/source/bwa-0.5.9/bwa samse /glusterfs/users/pmakella/RSQ6files/hg/refFiles/hg19.fa  $Dirname/$DataFile1.sai $DataFile  > $Dirname/$DataFile1.sam
...
R CMD BATCH -args --baseDir=$Dirname --outDir=$Dirname --factor=$DataFile1 --ipFile=$DataFileQ30.sam.bam  /glusterfs/users/pmakella/scripts/spp_for_wig_generation_no_ct_BAM.R
